...
\begin{frame}{Reverse Diffusion Process}
  ...
  \begin{block}{Theorem. Reverse of Gaussian DP is $\approx$ Gaussian DP\footcite{thermodynamic}}
    ...
    \]
  \end{block}
  \onslide<3->{\tikzmarkin[below right offset={-0.2,0.2}, above left
    offset={0.,-0.2}]{b} Mean $\mub_\theta$ and covariance
...
\begin{frame}{CLIP: Latent Embedding model}
  ...
  \begin{column}{0.5\textwidth}
    ...
    \item  The \textbf{Text Encoder} takes a text $y$ in
      and embeds it in $z\in\R^n$.
      \vspace{0.5cm}
    \item The \textbf{Image Encoder} takes an image $x$ and embeds it in the
      latent space $\R^n$.
      \vspace{0.5cm}
...
%  \footcite{clip}
%\end{frame}
\begin{frame}{DALL-E2 method}
  Let $(x,y)$ an image, caption pair.
  \begin{enumerate}
    \item \textbf{CLIP} such that $(x,y)\mapsto(z_i, z_t)$.
    \item \textbf{Prior} which estimate $q(\zz_i\,\vert\,\yy)$ \hfill
      \onslide<2->{\textit{\color{rred}Provide an image embedding conditioned
          to a text.}}
    \item \onslide<2->{\textbf{Decoder} which estimate $q(\xx\,\vert\, \zz_i,
      \yy)$}\hfill
